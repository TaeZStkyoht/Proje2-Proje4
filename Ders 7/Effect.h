...
public:
    ...
    virtual void draw(){
		...
		loadTexture("okuyom_ben_ya.jpg");
        glUniform1i(fbo_texture, 0);

		glActiveTexture(GL_TEXTURE1);
		loadTexture("ramp.bmp");
		glUniform1i(CorrectionColor, 1);
...
private:
	...
	GLuint fbo_texture, CorrectionColor;
    GLuint vbo_fbo_vertices;
    GLuint UniformVertex;
    ...
    void initShader(const char* fragmentShaderFile){
		...
		fbo_texture = glGetUniformLocation(programID, "fbo_texture");
        if (fbo_texture == -1)
		{
            fprintf(stderr, "Could not bind attribute %s\n", "fbo_texture");
        } 
        CorrectionColor = glGetUniformLocation(programID, "CorrectionColor");
        if (CorrectionColor == -1)
		{
            fprintf(stderr, "Could not bind attribute %s\n", "CorrectionColor");
        } 
    }
